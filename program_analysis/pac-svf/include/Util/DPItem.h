...
template<class LocCond>
class CSPL_CxtStmtDPItem : public CxtStmtDPItem<LocCond>{
    public:
    // static int cspl_count;
    // int cspl_id;
    ContextCond actual_context;
    // static OrderedMap<int,ContextCond> actual_contextMap;
    // ContextCond* actual_context;
    CSPL_CxtStmtDPItem(const CxtVar& var, const LocCond* locCond) : CxtStmtDPItem<LocCond>(var,locCond)
    {
        // cspl_count++;
        // cspl_id=cspl_count;
       actual_context=var.get_cond();
    //    actual_contextMap.insert(std::make_pair(cspl_id, actual_context));
        // actual_context=new ContextCond(var.get_cond());
    }
    //  inline bool operator== (const CSPL_CxtStmtDPItem<LocCond>& rhs) const
    // {
    //     return (this->cur == rhs.cur && this->curloc == rhs.getLoc() && this->context == rhs.context);
    // }

    /// Copy constructor
    CSPL_CxtStmtDPItem(const CSPL_CxtStmtDPItem<LocCond>& dps) :
        CxtStmtDPItem<LocCond>(dps)
    {
        // cspl_count++;
        // cspl_id=cspl_count;
        actual_context=dps.actual_context;
        // ContextCond actual_context(actual_contextMap[dps.cspl_id]);
        // actual_contextMap.insert(std::make_pair(cspl_id, actual_context));
    }
    /// Destructor
    virtual ~CSPL_CxtStmtDPItem()
    {
    }
