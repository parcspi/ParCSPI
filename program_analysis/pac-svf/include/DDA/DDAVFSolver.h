...
#ifndef CSPL_DIS_BUDGET
#define CSPL_DIS_BUDGET false
#endif
...
public:
    ...
    // DPMToDPMMap old_to_new_dpm_map;//should be new to old
    DPMToDPMMap oldStoreSrc_to_newDpm_map;//should be new to old
    DPMToCVarMap addrDpM_to_CVar_map;
    ///Constructor
    ...
    virtual const CPtSet& findPT(const DPIm& dpm)
    {
        ...
        if(testOutOfBudget(dpm) == false)//cspl_disable budget
        {
            // SVFUtil::outs() << "RRREE\n";
            CPtSet pts;
...
public:
     const DPIm* cspl_previousGepdpm = nullptr;
    ...
    virtual void reComputeForEdges(const DPIm& dpm, const SVFGEdgeSet& edgeSet, bool indirectCall = false)
    {
    ...
    ///@{
    virtual inline void startNewPTCompFromLoadSrc(CPtSet& pts, const DPIm& oldDpm)
    {
    ...
    virtual inline void startNewPTCompFromStoreDst(CPtSet& pts, const DPIm& oldDpm)
    {
    ...
    inline void backtraceToStoreSrc(CPtSet& pts, const DPIm& oldDpm)
    {
        ...
        DPIm dpm(oldDpm);
        dpm.setLocVar(edge->getSrcNode(),store->getPAGSrcNodeID());
        
        if(oldStoreSrc_to_newDpm_map.find(oldDpm) == oldStoreSrc_to_newDpm_map.end())
        {
            oldStoreSrc_to_newDpm_map.insert(std::make_pair(oldDpm,dpm));
        }else{
            // SVFUtil::outs()<<"dpm exist\n";
            // dpm.dump();
            // oldDpm.dump();
            // SVFUtil::outs().flush();
        }
    ...
    virtual void backwardPropDpm(CPtSet& pts, NodeID ptr,const DPIm& oldDpm,const SVFGEdge* edge)
    {
        ...
        // if(old_to_new_dpm_map.find(dpm) == old_to_new_dpm_map.end())
        // {
        //      old_to_new_dpm_map.insert(std::make_pair(dpm,oldDpm));
        //     //  [dpm] = oldDpm;
        // }else{
        //     SVFUtil::outs()<<"dpm already exist\n";
        //     dpm.dump();
        //     oldDpm.dump();
        //     SVFUtil::outs().flush();
        // }
...
protected:
    ...
    inline bool testOutOfBudget(const DPIm& dpm)
    {
        ...
        if(!CSPL_DIS_BUDGET&&++ddaStat->_NumOfStep > DPIm::getMaxBudget())
